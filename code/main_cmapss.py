...
if __name__ == '__main__':
    with open("../data/CMAPSS/trainX_cmapss", "rb") as fp:
        trainX = pickle.load(fp)
    with open("../data/CMAPSS/testX_cmapss", "rb") as fp:
        testX = pickle.load(fp)
    with open("../data/CMAPSS/trainY_cmapss", "rb") as fp:
        trainY = pickle.load(fp)
    with open("../data/CMAPSS/testY_cmapss", "rb") as fp:
    ...
    logh = open("../results/tables5_cmapss_res.txt", "w")
    sys.stdout = logh
    ...
    p = 0.8
    lbd = 0.09
    FL = FairCL(nsamples, 3, p, trainX, trainY, testX, testY)  # SSI: K=2, p=0.5, lbd=0.5 best
    FL.train(lbd, 10)
    out_FL = FL.test()
